...
class EasyNavigation private constructor() {
    ...
    private var mNavigationMode: NavigationMode = NavigationMode.SIMPLE_IMMERSION // 默认沉浸式 , 用于简单一种颜色
    ...
    class Builder(context: Context) {
        ...
        private var mDefaultView = LayoutInflater.from(context).inflate(R.layout.view_default_navigation, null, true)
        ...
        fun setViewWidgetClickListener(widgetID: Int, iViewWidgetClickListener: IViewWidgetClickListener) {
            ...
            if (mContext != null) {
                ...
                if (mEasyNavigation.mRootViewHeight != 0) {
                    mDefaultView.layoutParams =
                            ViewGroup.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT, mEasyNavigation.mRootViewHeight)
                } else {
                    mContext?.let {
                        mDefaultView.layoutParams =
                                ViewGroup.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT, DensityUtil.dip2px(it, 56f))
                    }
                }
                ...
                if (mEasyNavigation.mStatusBarColor != 0 && isSeparate() && isDefault()) {
                    mContext?.let {
                        StatusBarUtils.setStatusBarColor(
                            activity,
                            it.resources.getColor(mEasyNavigation.mStatusBarColor)
                        )
                    }
                }
                ...
                if (mEasyNavigation.mBackground != 0 && isDefault()) {
                    if (isImmersion()) {
                        ...
                        mContext?.let {
                            statusBarHeight = DensityUtil.getStatusBarHeight(it)
                        }
                ...
                if (mEasyNavigation.mTitleSize != 0 && isDefault()) {
                    ...
                    if (mEasyNavigation.mTitleMode == TitleMode.CENTER) {
                        ...
                        if (findDefaultWidgetByTag(Tag.TV_CENTER_TITLE) != null) {
                            title = findDefaultWidgetByTag(Tag.TV_CENTER_TITLE) as TextView
                            mContext?.let {
                                title.textSize = DensityUtil.dip2px(it, mEasyNavigation.mTitleSize).toFloat()
                            }
                        }
                    } else {
                        // 左边
                ...
                if (mEasyNavigation.mTvRightSize != 0 && isDefault()) {
                    if (findDefaultWidgetByTag(Tag.TV_RIGHT2) != null) {
                        val title: TextView = findDefaultWidgetByTag(Tag.TV_RIGHT2) as TextView
                        mContext?.let {
                            title.textSize = DensityUtil.dip2px(it, mEasyNavigation.mTvRightSize).toFloat()
                        }
                    }
                }
